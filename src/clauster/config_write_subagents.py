...
shadows one — :func:`_is_read_only_file` treats that as read-only too, using the same
two structural signals :mod:`clauster.config_write_hooks` uses for plugin-owned hook
commands: a symlink (a real project/user agent is always a plain file this surface
itself wrote) or the literal ``${CLAUDE_PLUGIN_ROOT}`` interpolation appearing
anywhere in the raw bytes (that interpolation only resolves inside a plugin's own
bundle). Either signal is sufficient, and both are content/metadata sniffs — nothing
is ever resolved, spawned, or executed to make this determination.
...
def _write_agent(root: Path, name: str, content: str, expected_hash: str | None) -> None:
    ...
    Gate order (mirrors the Foundation): built-in-name refusal → path containment →
    existing-file plugin-read-only refusal → structural validation (422, nothing
    written) → stale-hash guard (409) → atomic write via
    :func:`clauster.config_file_writer.write_file`. A name colliding with a built-in
    is refused *before* any I/O, and an existing plugin-owned file is refused before
    the new content is even validated — in neither case is anything ever written.
    """
    if is_builtin_agent(name):
...
def _delete_agent(root: Path, name: str) -> bool:
    ...
    yours to remove"). A plugin-owned on-disk file is likewise refused. A genuinely
    absent, ordinary name returns ``False`` (idempotent, matches
