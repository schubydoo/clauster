...
def create_setup_app(write_path: Path, *, port: int = DEFAULT_PORT) -> FastAPI:
    ...
    async def submit(request: Request) -> Response:
        ...
        data = _build_config_data(str(Path(pr_raw).expanduser()), host, port_val, password_hash)
        # Final fail-closed gate: never write a config that would not boot (e.g. a bad host,
        # or a non-loopback bind rule the model enforces). The friendly per-field checks above
        # catch the common cases; this catches the rest before anything lands on disk.
        try:
        ...
        try:
            atomic_write_text(write_path, content)
        except OSError as exc:
            return JSONResponse(
                {"detail": f"could not write {write_path}: {exc}"}, status_code=500
            )
