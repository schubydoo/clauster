...
| Owner-only file modes (`0o600` / `0o700`) | ✓ | ✓ | ✓ | POSIX sets `0700`/`0600` mode bits; Windows sets an equivalent owner-only ACL on the state dir via `icacls` (remove inheritance; grant only the current user + SYSTEM), fail-closed — so `session.secret` is owner-only regardless of where `state_dir` points. |
