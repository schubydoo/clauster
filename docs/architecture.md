...
| `poll_loop.py` | `PollLoop` — the poll + metrics background loops (liveness reconcile, `claude agents --json` cross-check, phantom-card prune, stuck-STARTING promotion, and the server-side metrics sampler), split from `runner.py` (#1157); built with the `RunnerState` + `RecordFacade` so every loop write and lifecycle emission lands on the one registry / record facade. `PollLoop` owns the two loop tasks; `SessionRunner` holds one instance, re-exposes the tasks + reconciled-session cache as proxy properties (so `shutdown()` still cancels + awaits both), and delegates. |
| `spawn_coordinator.py` | `SpawnCoordinator` — the spawn / resume path and the fail-closed pre-spawn gate sequence (trust check → remote-control enable → bridge cap → deferred trust write → subprocess), both bridge launches (standard `remote-control` and the pty keeper), the readiness waits, and the off-request startup watch, split from `runner.py` (#1157); built last with the `RunnerState` + `RecordFacade` + `BridgeLaunch` + `Rediscovery` so the launch, registry writes, and lifecycle emits all go through the one collaborator each. `stop`/`forget`/`adopt` stay on `SessionRunner` and keep taking the same shared locks; the coordinator holds no locks. `SessionRunner` holds one instance and delegates the public `spawn`/`spawn_detailed`/`resume`/`resume_detailed`. |
...
| `procutil.py` | `psutil`-based process introspection: liveness with PID-reuse defense (start-time pair, a per-boot id for the bridge, keeper, and hosted paths, and cmdline match) and the match-gated kill behind bridge rediscovery and hosted orphan recovery. |
| `auth.py` | Auth foundation (fail-closed; pure functions + the `LoginThrottle` limiter, no FastAPI import). |
