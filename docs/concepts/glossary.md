...
| **PTY keeper** | The small sidecar process that owns an Interactive Session's pseudo-terminal ("pty") and keeps it alive independently of Clauster. `clauster keepers` lists and stops orphaned ones. | [`clauster keepers`](../reference/cli.md#clauster-keepers-stop-an-orphaned-pty-keeper). |
...
| **claustrum** | A separate helper daemon (not a typo of "clauster") that carries Direct Sessions. One per deployment, started automatically when `claustrum.enabled` is set; it keeps running across Clauster restarts. | [Configuration](../reference/config.md). |
...
| **Permission mode** | How much a session may do without asking you first: `default`, `plan`, `acceptEdits`, `auto`, `dontAsk`, or `bypassPermissions`. These are Claude Code's own modes; `bypassPermissions` (no confirmation at all) is [footgun-gated](../security.md#bypasspermissions-footgun-gate). A seventh choice, `inherit`, is Clauster's own: it forces nothing, passing no `--permission-mode` flag so the session starts in its own default mode — which is whatever the session's settings files say, so see the [`inherit` caveat](../security.md#bypasspermissions-footgun-gate) before using it where `allow_bypass_permissions: false` matters. | Launch dialog; [Configuration](../reference/config.md). |
...
| Term | What it means | Where it's used |
| --- | --- | --- |
| **`projects_root`** | The directory whose child directories become the project cards on the dashboard. | [Configuration](../reference/config.md). |
| **`state_dir`** | Where Clauster keeps its own runtime state — the `clauster.db` database, bridge logs, and the claustrum socket. Default `~/.clauster`. | [Configuration](../reference/config.md); [Privacy](../privacy.md#at-rest-inventory). |
