...
| `allow_unauthenticated_network` | bool | `false` | Explicit opt-out: permit a non-loopback bind **without** enforced auth (e.g. a trusted LAN). `ops._check_auth` downgrades this to a warning. **Anyone who can reach the port then has full operator control of this host** — the dashboard drives a shell; treat it accordingly. |
