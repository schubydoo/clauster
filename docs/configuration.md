...
| `verbose` | bool | `false` | Pass `--verbose` to spawned remote-control bridges for detailed connection/session logs. Applies to the standard (multi-session) bridge only; off by default. |
