...
| Legacy state copies | `~/.clauster/state.json.bak`, `state.json.corrupt.bak` (and the `hosted_state.json` equivalents) | One-time copies of a legacy file Clauster could not act on as-is: `.bak` before it coerced a mismatched `schema_version` (older or newer), `.corrupt.bak` — a byte-exact copy — when the file could not be used as a store at all: undecodable bytes, unparseable JSON, or JSON that parses to the wrong shape. `.bak` holds the pre-coercion file's text; `.corrupt.bak` holds that file's bytes exactly as found, whatever they turned out to be. Either can carry the hosted store's `claude` session uuid, so both are written `0600` regardless of the mode the legacy file itself carried. | Taken at most once each and never rewritten. **Not** renamed `*.imported` with the original. Clauster never deletes them, so delete them yourself once you no longer need the recovery source — but note `clauster restore --force` puts the archive's `state_dir` back, so a copy the archive does not contain is gone (see [Recovering from a corrupted state database](operations.md#recovering-from-a-corrupted-state-database)). |
...
| Session transcripts | `~/.claude/projects/<sanitized-cwd>/<uuid>.jsonl` | Written by `claude`, **not** Clauster: the full conversation transcript per session. Clauster reads these in three places: the per-project cost / token badge, the read-only **Transcripts** viewer, and — **new in 1.1** — restoring a Direct Session's View pane when it reattaches after a restart. | Owned by the `claude` CLI; Clauster never deletes them. |
| Bridge pointer | `~/.claude/projects/<sanitized-cwd>/bridge-pointer.json` | A pointer `claude` writes linking a directory to its active session and environment. When Clauster must stop a restart from reattaching an archived session, it rewrites the file to keep only the environment id and the bridge pid, and saves a `bridge-pointer.json.bak` copy first. | Owned by the `claude` CLI. Clauster does not delete a pointer that has an environment id. For a non-live pointer older than 14 days, it removes the session id and keeps the environment id. |
