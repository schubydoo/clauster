...
| Legacy state copies | `~/.clauster/state.json.bak`, `state.json.corrupt.bak` (and the `hosted_state.json` equivalents) | One-time copies of a legacy file Clauster could not act on as-is: `.bak` before it coerced a mismatched `schema_version` (older or newer), `.corrupt.bak` — a byte-exact copy — when the file could not be parsed as JSON at all. Each holds the same fields as the legacy file it copied, including, for the hosted store, the `claude` session uuid, so both are written `0600` regardless of the mode the legacy file itself carried. | Taken at most once each and never rewritten. **Not** renamed `*.imported` with the original and never pruned — delete them yourself once you no longer need the recovery source (see [Recovering from a corrupted state database](operations.md#recovering-from-a-corrupted-state-database)). |
