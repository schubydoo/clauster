...
| Legacy state copies | `~/.clauster/state.json.bak`, `state.json.corrupt.bak` (and the `hosted_state.json` equivalents) | One-time copies of a legacy file Clauster could not act on as-is: `.bak` before it coerced a mismatched `schema_version`, `.corrupt.bak` when the file could not be read as JSON at all. Each holds the same fields as the legacy file it copied — including, for the hosted store, the `claude` session uuid. Written `0600`. | Taken at most once each and never rewritten. **Not** renamed `*.imported` with the original and never pruned — delete them yourself once you no longer need the recovery source. |
