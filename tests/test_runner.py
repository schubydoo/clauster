...
def test_state_hub_proxies_the_single_runner_state(runner_config):
    # #1157: the registry / crash tally / metrics cache / startup-watch tasks and the
    # persist mirror (_persisted/_row_backed/_last_saved) live on ONE RunnerState; the
    # runner's like-named attributes are thin proxies onto it. Guard the load-bearing
    # single-instance invariant: a second hub (or a diverged dict) would let spawn write
    # one registry while poll/persist read another, silently. Also confirm each seam
    # setter writes THROUGH to the collaborator every reader shares.
...
async def test_lock_facade_delegators_enter_the_runner_state_managers(runner_config):
    # #1157: the spawn/stop/forget/adopt/resume sites still take the locks off the runner
    # (`_spawn_lock_for` / `_bridge_flock` / `_store_flock` / `_flock`), now thin delegators
    # onto the ONE RunnerState. Drive each to completion so the façade seam is exercised end
    ...
    async with runner._store_flock():
        pass
    async with runner._flock((runner._config.state_dir / "facade-probe").expanduser()):
        pass
...
async def test_forget_refuses_when_keeper_process_still_live(runner_config, monkeypatch):
    ...
    inst.keeper_proc_start = 12345.0
    inst.keeper_start_ticks = 770579
    # Gated on the (pid, start-identity) match now, not bare "any process alive at this pid"
    # nor cmdline alone: a recycled pid must not strand a record, since forget never kills
...
def test_reattach_pty_from_sidecar_refuses_a_non_pty_row(runner_config, monkeypatch):
    ...
    assert runner._reattach_pty_from_sidecar("alpha", saved, exclude_pids=frozenset()) == []
...
def test_reattach_pty_from_sidecar_autogenerates_an_instance_id(runner_config, monkeypatch):
    ...
    [inst] = runner._reattach_pty_from_sidecar("alpha", saved, exclude_pids=frozenset())
