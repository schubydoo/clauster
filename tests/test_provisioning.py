...
def test_clone_rename_conflict_is_target_exists(tmp_path, monkeypatch):
    ...
    with pytest.raises(TargetExists):
        clone_project(tmp_path, "proj", "https://10.0.0.1/r.git",
                      cfg=_cfg(allow_private_hosts=True), git_binary=_gitbin())
...
def test_clone_success_and_transport_env(tmp_path, monkeypatch):
    monkeypatch.setenv("FAKE_GIT_MODE", "ok")
    dest = clone_project(
        tmp_path, "proj", "https://10.0.0.1/r.git",
        cfg=_cfg(allow_private_hosts=True), git_binary=_gitbin(),
    )
...
def test_clone_shallow_passes_depth(tmp_path, monkeypatch):
    ...
    dest = clone_project(
        tmp_path, "proj", "https://10.0.0.1/r.git",
        cfg=_cfg(allow_private_hosts=True), git_binary=_gitbin(), shallow=True,
    )
...
def test_clone_timeout(tmp_path, monkeypatch):
    ...
    with pytest.raises(CloneFailed):
        clone_project(tmp_path, "proj", "https://10.0.0.1/r.git",
                      cfg=_cfg(allow_private_hosts=True, timeout_seconds=1), git_binary=_gitbin())
    assert not (tmp_path / "proj").exists()
...
def test_clone_size_cap(tmp_path, monkeypatch):
    ...
    with pytest.raises(CloneFailed):
        clone_project(tmp_path, "proj", "https://10.0.0.1/r.git",
                      cfg=_cfg(allow_private_hosts=True, max_mb=1), git_binary=_gitbin())
    assert not (tmp_path / "proj").exists()
...
def test_route_clone_success(write_config, monkeypatch):
    ...
    resp = client.post("/api/projects/clone", json={"name": "cloned", "url": "https://10.0.0.1/r.git"})
    assert resp.status_code == 201
    body = resp.json()
