...
async def test_start_never_cards_a_second_bridge_on_a_pty_moded_pointer_card(
    ...
    # A stopped card that still records pid 9001 (`stop()` leaves it) goes FIRST in the
    # registry, so a scan that ignored the status would hand it back.
    stale = RemoteControlInstance(
        project="alpha", label="stale", status=InstanceStatus.STOPPED, bridge_pid=9001
    )
    rest = dict(runner._instances)
    runner._instances.clear()
    runner._instances[stale.instance_id] = stale
    runner._instances.update(rest)
    ...
    holders = [
        i
        for i in runner.list_instances()
        if i.bridge_pid == 9001 and i.status is InstanceStatus.RUNNING
    ]
    assert holders == [card], f"two cards drive pid 9001: {holders}"


def _held_card(**kw) -> RemoteControlInstance:
    fields = {"project": "alpha", "label": "alpha", "status": InstanceStatus.RUNNING}
    return RemoteControlInstance(**{**fields, "bridge_pid": 9001, **kw})
...
def test_live_card_for_pointer_matches_the_pid_and_start_pair(
    ...
    got = runner._rediscovery._live_card_for_pointer(ptr)

    assert (got is card) is hit


async def test_external_reattach_refuses_a_bridge_another_projects_card_holds(
    runner_config, monkeypatch
):
    ...
    runner = _make_runner(runner_config)
    monkeypatch.setattr("clauster.procutil.jiffies_to_epoch", lambda j: 500.0)
    foreign = _held_card(project="beta", label="beta", bridge_start_ticks=1000)
    ...
    monkeypatch.setattr(
        "clauster.runner.pointers.pointer_for_project",
        lambda p: _Ptr() if p.name == "alpha" else None,
    )
    monkeypatch.setattr("clauster.runner.procutil.is_live_standard_bridge", lambda *a, **k: True)
    monkeypatch.setattr(
        "clauster.runner.procutil.proc_cwd",
...
async def test_second_restart_still_hides_the_pid_less_row_of_a_live_pty_session(
    ...
    monkeypatch.setattr(Rediscovery, "_recover_keeper_pid", lambda self, n, p, s, **_kw: 5555)
    ...
    runner.persistence.state_store().save(
        {
            ...
            "iid-pidless": _row("alpha", pid=None, label="session-B"),
        }
    )
    _write_keeper_sidecar(
...
async def test_rediscover_drops_a_keeper_pid_that_is_no_longer_a_keeper(
    ...
    monkeypatch.setattr(Rediscovery, "_recover_keeper_pid", lambda self, n, p, s, **_kw: None)
    await runner.rediscover(persist=False)
...
async def test_pid_less_pty_row_is_not_carded_while_a_live_keeper_is_unclaimed(
    ...
    runner = _make_runner(runner_config)
    monkeypatch.setattr(Rediscovery, "_has_unclaimed_live_keeper", lambda self, name, held: True)
    await runner.rediscover(persist=False)
    ...
    runner2 = _make_runner(runner_config)
    monkeypatch.setattr(Rediscovery, "_has_unclaimed_live_keeper", lambda self, name, held: False)
    monkeypatch.setattr("clauster.runner.procutil.is_live_bridge", lambda *a, **k: False)
    await runner2.rediscover(persist=False)
...
async def test_pid_less_pass_leaves_a_pid_bearing_row_to_the_row_pass(runner_config, monkeypatch):
    ...
    monkeypatch.setattr(Rediscovery, "_reattach_rows_with_pids", _no_rows)
    monkeypatch.setattr(Rediscovery, "_stopped_from_persisted", lambda self, name: None)
...
async def test_pointer_walk_reattach_records_the_keeper_start_time_too(runner_config, monkeypatch):
    ...
    monkeypatch.setattr(Rediscovery, "_recover_keeper_pid", lambda self, n, p, s, **_kw: 6666)
...
async def test_resync_replaces_the_keeper_pair_together(runner_config, monkeypatch):
    ...
    _stub_connect(monkeypatch)
    monkeypatch.setattr(Rediscovery, "_recover_keeper_pid", lambda self, n, p, s, **_kw: 2222)
...
async def test_a_sweep_that_raises_blocks_that_project_instead_of_killing_startup(
    ...
    monkeypatch.setattr(Rediscovery, "_has_unclaimed_live_keeper", _boom)
...
async def test_row_arriving_during_the_sweep_is_deferred_not_carded(runner_config, monkeypatch):
    ...
    monkeypatch.setattr(Rediscovery, "_modes_with_an_unclaimed_live_bridge", _inject)
    await runner.rediscover(persist=False)
...
async def test_poll_adoption_skips_when_the_store_read_fails(runner_config, monkeypatch):
    ...
    monkeypatch.setattr(runner._registry, "_refresh_persisted", _failed_refresh)
    await runner.poll_once()
...
async def test_resync_replaces_the_dead_generations_connect_facts(runner_config, monkeypatch):
    ...
    monkeypatch.setattr(Rediscovery, "_connect_facts_for", lambda *a, **k: {})
...
async def test_resync_aborts_when_the_row_is_forgotten_while_recovering_facts(
    ...
    def _forget_mid_flight(self, proj, mode, pid, start, **_kw):
        self._registry._persisted.pop("iid-a", None)  # another process's `clauster forget`
        return {"url": "https://claude.ai/code?environment=env_STUB"}

    monkeypatch.setattr(Rediscovery, "_connect_facts_for", _forget_mid_flight)
...
async def test_adoption_does_not_overwrite_an_instance_a_lock_holder_created(
    ...
    def _land_concurrently(self, proj, mode, pid, start, **_kw):
        ...
        self._registry._instances["iid-a"] = winner
        return {"url": "https://claude.ai/code?environment=env_STUB"}

    monkeypatch.setattr(Rediscovery, "_connect_facts_for", _land_concurrently)
    await runner.poll_once()
...
async def test_adopted_starting_bridge_promotes_once_connect_evidence_lands(
    ...
    evidence: dict = {}
    monkeypatch.setattr(Rediscovery, "_connect_facts_for", lambda *a, **k: dict(evidence))
...
async def test_promotion_skips_a_row_whose_generation_changed_mid_probe(
    ...
    monkeypatch.setattr(Rediscovery, "_connect_facts_for", _resume_lands)
...
async def test_promotion_skips_a_bridge_that_died_during_the_evidence_read(
    ...
    monkeypatch.setattr(Rediscovery, "_connect_facts_for", _dies_during_read)
    inst = _adopted_starting(runner)
...
# -- #1438: a screen-fault notice + RUNNING status survive a restart+reattach ----------


async def test_reattached_ready_row_with_no_link_keeps_the_notice_and_runs(
    runner_config, monkeypatch
):
    # #1438. A pty session that captured no connect link under a screen fault is promoted to
    # `ready` by the keeper with `connect_url: null` and an advisory `note`. The spawn path
    # (`_apply_pty_info`) reads that note onto the card as `notice` and reports RUNNING. After
    # a Clauster restart the row carries a live pid, so `_reattach_rows_with_pids` rebuilds it
    # through the REAL `_connect_facts_for` — the leg that used to skip the note and return an
    # empty dict for a linkless-but-ready sidecar, so the card flipped to STARTING and lost
    # the reason. Both must now survive: RUNNING, and the notice intact.
    from clauster.pty_keeper import _SCREEN_FAULT_NOTE

    runner = _make_runner(runner_config)
    # Deliberately NOT `_stub_connect`: the whole point is to drive the real recovery over a
    # ready sidecar that carries a note and no link.
    runner.persistence.state_store().save({"iid-fault": _row("alpha", pid=6001, proc_start=111.0)})
    runner._log_dir.mkdir(parents=True, exist_ok=True)
    (runner._log_dir / "alpha-1700000000000-0.keeper.json").write_text(
        json.dumps(
            {
                ...
                "state": "ready",
                "connect_url": None,  # the screen fault: no link captured
                "session_id": None,
                "note": _SCREEN_FAULT_NOTE,
            }
        )
    )
    # The row's own bridge is live, so `_reattach_rows_with_pids` handles it (not the sidecar
    # walk). Keeper recovery is stubbed live for realism; it does not gate the notice/status.
    monkeypatch.setattr(
        "clauster.runner.procutil.is_live_bridge", lambda pid, *a, **k: pid == 6001
    )
    monkeypatch.setattr("clauster.runner.procutil.is_keeper_process", lambda pid: pid == 5555)
    monkeypatch.setattr("clauster.runner.pointers.pointer_for_project", lambda path: None)
    ...
    inst = {i.instance_id: i for i in runner.list_instances()}["iid-fault"]
    assert inst.status is InstanceStatus.RUNNING, "a ready row with no link must run, not STARTING"
    assert inst.notice == _SCREEN_FAULT_NOTE, "the screen-fault notice was lost across restart"
    assert inst.url is None  # the link really is gone; the notice is the whole story


async def test_promote_ready_unwatched_lifts_the_screen_fault_notice(runner_config, monkeypatch):
    # #1438, the SECOND promotion leg. A pty row reattached while its keeper was still starting
    # comes back STARTING with no watch of its own; `_promote_ready_unwatched` re-reads the
    # sidecar on a later tick. When the keeper then hits a screen fault (`ready`, no link, a
    # note), `_connect_facts_for` returns a notice-only dict — this leg must promote to RUNNING
    # AND copy the notice, or the card goes green with no link and no reason (the exact #1438
    ...
    runner._log_dir.mkdir(parents=True, exist_ok=True)
    (runner._log_dir / "alpha-1700000000001-0.keeper.json").write_text(
        json.dumps(
            {
                "keeper_pid": 5556,
                "bridge_pid": 6002,
    ...
    monkeypatch.setattr(
        "clauster.runner.procutil.is_live_bridge", lambda pid, *a, **k: pid == 6002
    )
