...
async def test_resume_gate_fails_closed_when_refresh_fails(runner_config, monkeypatch):
    ...
    monkeypatch.setattr(runner._registry._store, "load_strict", _boom)
...
async def test_persist_holds_the_store_wide_flock_across_the_save(runner_config, monkeypatch):
    ...
    config, claude_json = runner_config
    runner = SessionRunner(config, claude_json=claude_json)
    real_save = runner._registry._store.save
    observed = {}
    ...
    monkeypatch.setattr(runner._registry._store, "save", _save_and_probe)
    own = _fake_instance("alpha")
    runner._instances[own.instance_id] = own
    await runner._persist()
